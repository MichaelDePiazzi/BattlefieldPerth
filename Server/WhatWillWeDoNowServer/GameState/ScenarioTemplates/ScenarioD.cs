...
{
    public class ScenarioD : Scenario
    {
        public ScenarioD()
        {
            Id = "D";
            Title = "Forrest Chase";
            ImageIndex = (int)GameState.ImageIndex.ScenarioD;
        ...
        private static Outcome CreateOutcome5()
        {
            return new Outcome
            {
                ...
                ActionOutcomeAndGetDisplayText = players =>
                {
                    GameStateManager.DamagePlayers(players, player => (player.SelectedChoice == Choice.A) || (player.SelectedChoice == Choice.B));
                    GameStateManager.DamagePlayers(players, player => (player.SelectedChoice == Choice.B));
                    return "As you sneak towards the large shopping centre, the guards scan the area, and open fire at anyone not keeping low. The flechettes sink deeply into the flesh of anyone out in the open. You all race into the large store, and hide inside. The aliens make a token effort to search, but return outside to guard the pods.";
                },
                NextScenarioKey = "G"
            };
        }
